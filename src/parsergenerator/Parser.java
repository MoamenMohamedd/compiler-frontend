...
//    	
    	...
    	{
    		if(stack.peek()=="$"&&input.get(this.iterator)=="$")        //Both the stack and the input string  are empty
    			{
    		...
    		if(stack.peek()==input.get(this.iterator))							//A match do exists for 2 terminals
    			{
  		...
  		else if(!parseTable.isTerminal(stack.peek()))					//the top of the stack is a non terminal  
   		{
    		
    		
    		
			    			ArrayList<String> production=new ArrayList<>();
			    			production= (ArrayList<String>) this.parseTable.get(stack.peek(), input.get(this.iterator)) ;
			    			
			    			if(production.size()==0)	//Recovery from error Synch
			    			{
			    				stack.pop();
			    				System.out.println("ERROR");
			    			}
			    			else if(production.size()>0&& production.get(0)=="epsilon")
			    			{
			    				temp=stack.pop();
			    				System.out.print(temp+" ->"+"epsilon");
